...
import requests
import boto3
from util import get_logger, get_api_key, check_aws_creds, get_git_creds, get_assignment, get_submission_dir, get_submissions, get_submission_dir, get_runtime_env
...
def get_homework_prompt(s3_solutions_dir: str, local_solutions_dir: str, submission_filename: str) -> dict:
  ...
  if not os.path.exists(local_path):
    raise ValueError(f"Path does not exist: {local_path}")
  homework_prompt = open(local_path, "r").read().format(student_schema='juliescherer')
